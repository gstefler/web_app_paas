...
const displayedImages = computed(() => {
    const filtered = props.images.filter((img) => img.name.toLowerCase().includes(searchQuery.value.toLowerCase()));

    return filtered.sort((a, b) => {
        const aValue = sortBy.value === 'created_at' ? new Date(a.created_at).getTime() : a.name.toLowerCase();
        const bValue = sortBy.value === 'created_at' ? new Date(b.created_at).getTime() : b.name.toLowerCase();
    ...
    <div class="container mx-auto space-y-8 py-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-2 items-center">
                <AppearanceTabs/>
                <Button variant="outline" @click="logout" class="flex items-center gap-2">
                    <svg
        ...
        <Card>
                ...
                <form @submit.prevent="uploadImage" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center">
                            <input id="image" ref="fileInput" type="file" accept="image/*" @change="handleFileChange" class="hidden" />
            ...
            <CardFooter>
                <Button type="button" :disabled="!form.image || !form.name" @click="uploadImage"> Upload Image </Button>
            </CardFooter>
        </Card>
        ...
        <div class="flex items-center justify-between">
            <div class="relative">
                ...
                <Input v-model="searchQuery" placeholder="Search images..." class="w-[250px] pl-10" />
            </div>
        ...
        <Table>
            ...
            <TableBody>
                <TableRow v-for="image in displayedImages" :key="image.id">
                    <TableCell>
                        <img :src="image.path" :alt="image.name" class="h-10 w-10 cursor-pointer rounded object-cover" @click="viewImage(image)" />
                    </TableCell>
        ...
        <Dialog :open="showImageDialog" @update:open="showImageDialog = $event">
            <DialogContent class="max-w-3xl">
                ...
                <div class="mt-4">
                    <img v-if="selectedImage" :src="selectedImage.path" :alt="selectedImage.name" class="max-h-[70vh] w-full object-contain" />
                </div>
            </DialogContent>
        </Dialog>
    </div>
